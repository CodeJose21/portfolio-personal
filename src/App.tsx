  ...
  return <>
    ...
    <main id="main">
      <section id="home" className="hero section">
        ...
        <div className="portrait-wrap"><div className="portrait-frame"><img src={publicAsset('jose-gonzalez.jpg')} alt={t.portrait} width="600" height="670" fetchPriority="high"/><div className="portrait-label"><span className="status-dot"/> {t.portraitMotto}</div></div><div className="portrait-caption"><span>{t.portraitCaption}</span><Sparkles size={18}/></div><p className="photo-note">{t.photoCaption}</p></div>
        <div className="languages"><span className="language-label">{t.languageHeading}</span>{t.languages.map((language, i) => <div className="language" key={language}><img className="language-flag" src={publicAsset(`flags/${['es', 'gb', 'de'][i]}.png`)} alt="" width="32" height="32"/><strong>{language}</strong><span>{t.levels[i]}</span></div>)}</div>
      </section>
      ...
      <section id="personal" className="section personal-section"><div><p className="eyebrow">{t.personalEyebrow}</p><h2>{t.personalTitle}</h2><p>{t.personalText}</p><div className="personal-tags">{t.personalTags.map(tag => <span key={tag}>{tag}</span>)}</div></div><figure className="personal-photo">{personalPhoto ? <img src={publicAsset(personalPhoto)} alt={t.personalPhoto} loading="lazy" width="640" height="480"/> : <div className="personal-photo-placeholder"><Camera size={32} aria-hidden="true"/><span>{t.personalPhotoPending}</span></div>}<figcaption>{t.personalPhoto}</figcaption></figure></section>
    </main>
